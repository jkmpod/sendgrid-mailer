# Build History — Session Prompts

Archived prompts used to construct the project across seven sessions. They are
preserved for historical reference only — they do not describe the current
codebase. For the live picture, see:

- `README.md` — user-facing intro and endpoints
- `ARCHITECTURE.md` — package DAG, scope, dependency policy
- `AGENTS.md` — review checklists
- `DEVELOPING.md` — recipes for changing the code

> Sessions 4 and 5 in the original CLAUDE.md were duplicates; only one copy is
> kept here.

---

## Ground rules (original session-time)
...
## Session 1 — foundation packages (config, models, loader)
...
> Note: Sessions 4 and 5 in the original CLAUDE.md contained the same prompt;
> only one copy is preserved here.
...
## Session 7 — log panel state machine
